        ...
        *,
        ...
        for pressure in sorted(pressures, key=lambda item: item.oldest_enqueued_at):
            ...
            if not model_workers:
                start = self._smallest_fitting(candidates, free)
                if start:
                    actions.append(StartPlacement(start[0], start[1], "cold_backlog"))
                    return actions
            ...
            ready_workers = [worker for worker in model_workers if worker.state is RuntimeState.READY]
            ...
            if desired <= len(model_workers):
                continue
            start = self._smallest_fitting(one_gpu_profiles, free)
            if start and self._replica_has_useful_margin(start[0], ready_workers):
                actions.append(StartPlacement(start[0], start[1], "replica_backlog"))
                return actions
        return actions
        ...
        *,
        ...
        for profile in sorted(candidates, key=lambda item: item.gpu_count):
            for required_set in profile.eligible_gpu_sets:
                ...
                if not blockers or any(worker.state is RuntimeState.DRAINING for worker in blockers):
                    continue
                ready_times = [worker.ready_at for worker in blockers if worker.ready_at]
                fair_wait_started = max(
                    pressure.oldest_enqueued_at,
                    max(ready_times) if ready_times else now,
                )
                starvation_override = (
                ...
                if not starvation_override:
                    protected = [
                        ...
                        if worker.ready_at
                        and (now - worker.ready_at).total_seconds() < self.minimum_residency_seconds
                    ]
                    if protected:
                        continue
                    if all(worker.model_id in pressure_by_model for worker in blockers):
                        # Useful resident placements are replaced early only by a clear score win.
                        incoming = profile.predicted_tokens_per_second
                        outgoing = sum(
                            worker.profile.predicted_tokens_per_second for worker in blockers
                        )
